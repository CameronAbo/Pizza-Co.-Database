...
ORDER BY o.OrderID, oi.LineNumber;
...
-- 5. Display total number of employees at each store
SELECT
    s.StoreID,
    s.City,
    s.Address,
...
-- 7. Generate report for most popular products ordered
SELECT
    p.ProductID,
    p.ProductName,
...
-- 8. Display recipe ingredients required for each product
SELECT
    p.ProductName,
...
JOIN PAYMENT_METHOD pm ON pay.PaymentMethodID = pm.PaymentMethodID
ORDER BY o.OrderID;
...
-- 11. Display inventory usage required by each order
SELECT
    o.OrderID,
    p.ProductName,
    ii.ItemName,
    ...
    r.QuantityRequired,
    r.UnitAmount,
    (oi.Quantity * r.QuantityRequired) AS TotalIngredientNeeded
FROM ORDERS o
JOIN ORDER_ITEM oi ON o.OrderID = oi.OrderID
JOIN PRODUCT p ON oi.ProductID = p.ProductID
JOIN RECIPE r ON p.ProductID = r.ProductID
JOIN INVENTORY_ITEM ii ON r.InvItemID = ii.InvItemID
ORDER BY o.OrderID, p.ProductName, ii.ItemName;